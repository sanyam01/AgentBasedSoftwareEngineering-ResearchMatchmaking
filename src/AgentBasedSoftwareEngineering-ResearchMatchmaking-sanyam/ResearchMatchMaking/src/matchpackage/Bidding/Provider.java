...
//                msg.addReceiver(new AID("provider", AID.ISLOCALNAME));
                    ...
                    case 2:
                        ...
                        if (acl2 != null) {
                            System.out.println("[ContractReceiver] Received a contract");
                            System.out.println("[ContractReceiver] Accepted or Refused: " + acl2.getContent());
                            MessageTemplate template =
                                    MessageTemplate.MatchPerformative(ACLMessage.INFORM);
//                        ACLMessage msg = parent.receive(template);

                            reply2 = new ACLMessage(ACLMessage.ACCEPT_PROPOSAL);

//                msg.addReceiver(new AID("provider", AID.ISLOCALNAME));
                            reply2.addReceiver(acl2.getSender());
                            reply2.setContent("contract already created");
                            myAgent.send(reply2);
                            step = 3;
